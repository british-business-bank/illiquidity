---
title: "Discounting for Illiquidity in Unlisted Shares <img src=\"logo.svg\" style=\"float: right;\"/> "
subtitle: "Conventions, Controversies, and Contemplations"
author: "Matthew Maddison, Lee Bowell, Anthony Tanaka"
date: "12/9/2024"
...
Liquidity is a complex and abstract concept. Simply put, it refers to the flexibility and ease of trading an asset with minimal price impact. Liquidity has wide ranging applications and implications in finance from market microstructure to financial market stability. It is particularly pertinent in the valuations of hard-to-trade and unlisted securities, such as private company shares. It is widely agreed that liquidity is valuable; the disagreement lies in how to quantify the value of liquidity or, conversely, cost of illiquidity. This report reviews the relevant literature on unlisted equity discounts, opines on their efficacy, chooses our preferred methods, and proposes our alternatives. On the one hand, we agree with the consensus that Finnerty's Asian put option is preferred if an *off the shelf* option-theoretic approach is to be chosen, but on the other hand, we boldly argue that all the commonly used option models fail to capture the essence of illiquidity and that synthetic bid-ask spreads are our cleanest dirty shirt.

## Introduction
Although closely intertwined and often used interchangeably, marketability and liquidity are different concepts according to the [IPEV Valuation Guidelines](https://www.privateequityvaluation.com/Portals/0/Documents/Guidelines/IPEV%20Valuation%20Guidelines%20-%20December%202022.pdf). IPEV states that a valuation discount for marketability is not appropriate, wheras one to account for illiquidity is appropriate. 
...
IPEV defines marketability as the transaction time typically required for the type of asset under consideration. Fair value estimation is premised on an orderly transaction, not a forced liquidation, where the required time to consummate, how ever long it might be, is already taken into account. In other words, IPEV stipulates that factors related to a specific investor, such as his deal timeline, should not be reflected while factors related to the underlying asset, such as liquidity and voting rights, should be. That is, just because unlisted shares take longer to market and transact than listed shares does not in itself warrant a discount, according to IPEV. It is their liquidity characteristics that justify a discount. 

Without suggesting particular methodologies in the guidelines, the application of illiquidity discount is left to the practitioner's judgement, but option pricing is put forward as a potential approach. Practitioners tend to apply an ad-hoc discount in the range of 15-35%, which is based primarily on historical private placements. Alternative methods, especially option-theoretic approaches, have been proposed in recent years. Whatever the chosen method, we think it should ideally be theoretically sound, statistically stable, intuitive, parsimonious with observable variables, time-varying, and able to produce varying results for different companies. 

## Stylized Facts of Liquidity
Liquidity is like oxygen; we all know it is needed, but we don't know how much is needed at what time and how to put a price on it. To gauge its value, we must first define precisely what we mean by illiquidity. We view illiquidity essentially as a constraint on an investor's decision set related to selling the asset whose price follows some random process such as Brownian motion.   
...
If the investor can realise present value $A$ under $\Omega$, but only $B$ under $\psi$, where $A \geq B$ ^[It is entirely possible that $B > A$ as flexibility in a decision does not guarantee optimality but we ignore such cases for simplicity.], then $(A-B)$ can be viewed as the cost of illiquidity and $\frac{A-B}{A}$ the illiquidity discount. $A$ and $B$ can be calculated using a binomial tree representing the investor's real options under his decision sets at time $T_0$. 

While we think this perspective succinctly captures the nature of illiquidity, the lack of a generally optimal decision set renders it impracticable ^[It is similar to money, which more is unambiguously better, but the incremental value depends on the person's subjective utility function, which resembles the investor's subjective decision function here.]. Nonetheless, it illustrates that different investors with different horizons and decision sets value liquidity differently, and that the estimated illiquidity discount can be viewed as the equilibrium cost of restrictions resulting from such hypothetical decision set interactions, reflecting an appropriate fair value adjustment.
...
## Literature Review
Methodologies can be generally categorized as either empirical or theoretical. Empirical approaches focus on realized market data from private placements, pre-IPO trades, and cross-sectional statistical analyses. One then has to extrapolate from the approach to his particular assignment, which can be quite tenuous. 

Theoretical approaches are preferred if available as they specify a parsimonious set of parameters and tend to be logically explainable.
...
### Cross-Sectional Liquidity Return Factor

### Cross-Sectional Bid-Ask Spreads
The bid-ask spread is a key source of profit for making a market in a stock; it represents an option written by the market maker to a liquidity taker to buy at the ask and to sell at the bid. 
...
### Option-Based
Option-theoretic approaches to estimate the illiquidity discounts have been created and promulgated in recent years. They are primarily based around the idea of owning a put option underwritten by a hypothetical creditworthy counterparty making a market on an illiquid stock, such that:
...
\end{equation}
It follows that liquidity premium $=$ put premium or, conversely, illiquidity discount $=$ cost of put. The estimation then centers on option pricing. These approaches have become increasingly popular among practitioners. 

#### Chaffe's Plain Vanilla Put
@chaffe1993 was the first to introduce the concept of equation $\eqref{eq:chaffe}$. He stated that by purchasing a put option, the illiquid stock owner has effectively purchased liquidity as the option underwriter is now his liquidity provider. The price of the put is then the illiquidity discount. 
...
### Our Opinions
We think option-theoretic approaches are inadequate and overstate the illiquidity discounts. We agree with others' criticism that by neglecting upside retention, a put option alone fails to properly represent the underlying illiquidity problem. We think equation $\eqref{eq:chaffe}$, the foundation of option-theoretic models, is flawed; hence, we boldly argue that their estimated values being close to actual private placement data is more a coincidence rather than logical necessity. By truncating downside risk but leaving upside potential intact, in the case of Chaffe, the investor's payoff is shifted from a long $100 \delta$ stock position to one of long $50 \delta$ call with breakeven at spot $+$ premium.  
...
Based on our definition of illiquidity discounts in terms of a constrained decision set, unless $\Omega$ is a function of either $\delta, \gamma, \nu$, we think options fail to capture the nature of liquidity or liquidity risk under normal conditions. In brief, we see liquidity risk as a standalone risk from option Greeks. 

## Our Proposals

### Cliquet Put Options with Random Start Time
### Synthetic Bid-Ask Spreads with Volatility States


## Further Works
Given the breadth of the subject, there are inevitably many relevant literature and methodologies that we have missed. We have yet been able to review the *Quantitative Marketability Discount Model* proposed by Mercer Capital as well as many CAPM-based approaches. A wider range of methodologies should be included in future reviews. 

Our proposed cliquet option methodology is by and large a work in progress, to say the least. Given liquidity-based derivatives remain conceptual and undeveloped, our pricing methodology will take considerable time to develop and put to applications.

In practice, bid-ask quotes are only valid for small trades. Sales of meaningful sizes inevitably occur at inferior prices as quotes are adjusted downward for market impact, inventory accumulation, and adverse selection. Therefore, our proposed synthetic bid-ask spread model is believed to materially underestimate the illiquidity cost in private company shares. Further works including market microstructure should improve the estimation. 
